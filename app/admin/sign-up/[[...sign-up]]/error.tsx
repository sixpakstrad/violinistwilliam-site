...
type AdminSignUpErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function AdminSignUpError({
  error,
  reset,
}: AdminSignUpErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
      ...
      <section className="mx-auto max-w-3xl border border-ivory/10 bg-[#2f251f] p-8 shadow-[0_28px_90px_rgba(0,0,0,0.32)] sm:p-10">
        ...
        <h1 className="mt-5 font-display text-4xl leading-tight sm:text-5xl">
          {isClientTrustError
            ...
            : "The sign-up screen needs a refresh."}
        </h1>
        <p className="mt-5 text-base leading-8 text-ivory-muted">
          {isClientTrustError
            ...
            : "Something interrupted the secure sign-up flow. Try reopening the sign-up screen."}
        </p>
        {isClientTrustError ? (
